import type { AiProvider, Answer, KodaAiInput, KodaAiResult, KodaPlan, Question } from '../types/koda';

export const firstQuestion: Question = {
  eyebrow: 'Точка А',
  question: 'Что сейчас больше всего мешает двигаться?',
  placeholder: 'Долги, хаос, усталость, расфокус, работа не растет...',
...
const questionFlow: Question[] = [
  ...
  },
  {
    ...
    rationale: 'Это помогает выбрать механику шага: микро-старт, ритм, поддержка или снижение страха.',
  },
  {
  ...
  },
  {
    ...
    rationale: 'Без энергии даже правильные цели становятся тяжелыми.',
  },
  {
    eyebrow: 'Опора',
  ...
  },
  {
    ...
    question: 'По каким признакам ты поймешь: “я пришел к этой версии себя”?',
...
export function buildRulesPlan(input: KodaAiInput): KodaPlan {
  ...
  const goals = [
    {
      ...
      firstStep: hasCareer ? 'Изучить SQL 45 минут и сохранить один заметный результат.' : 'Выбрать один навык для роста и дать ему 30 минут.',
    },
    {
      title: 'Финансовая устойчивость',
      target: hasDebt ? 'Снизить давление долгов, увидеть расходы и начать подушку.' : 'Собрать понятную картину денег и снизить финансовый шум.',
      firstStep: 'Выписать обязательные платежи за месяц.',
    },
    {
      title: 'Энергия и здоровье',
      target: hasHealth ? 'Сон, вес и привычки должны поддерживать путь.' : 'Поднять базовую энергию, чтобы цели не держались на героизме.',
      firstStep: 'Выбрать один восстановительный ритуал на завтра.',
    },
    {
      title: 'Отношения и опора',
      target: hasFamily ? 'Больше спокойного времени с важными людьми.' : 'Добавить поддержку и меньше тащить путь в одиночку.',
  ...
  return {
    userProfile: {
      ...
      strategy: 'KODA будет давать маленькие действия на 15-60 минут, фиксировать прогресс и снижать трение перед началом.',
    },
    ...
    goals,
    nextStep: hasCareer ? 'Изучить SQL 45 минут' : 'Сделать один 30-минутный шаг к главной цели',
    explanation: 'Fallback rulesProvider использовал ответы, ключевые темы и простой профиль движения.',
    progressReward: '+0.12% к версии себя',
    visualPrompt: 'Текущий персонаж постепенно идет к спокойной уверенной версии себя в светлом рабочем пространстве.',
  };
}
